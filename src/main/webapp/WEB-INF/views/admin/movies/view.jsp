...
<!--  <div style="margin: 10px;">
 <div class="easyui-panel" style="height: 100%; width: 100%" data-options="collapsible:true" title="记事本">
	...
	<div class="easyui-panel" style="width: 100%;height: 100% " data-options="collapsible:true" title="视频管理">
		<div id="search">
		    <a href="javascript:openAdd()" class="easyui-splitbutton" iconCls="icon-add">上传视频</a>
		...
		<table class="easyui-datagrid" id="grid" data-options="pagination:true,rownumbers:true,singleSelect:true,toolbar:'#bar',loader:loadData">
			<thead>
				<tr>
					...
					<th data-options="field:'rtmp'">影片地址</th>
					<th data-options="field:'_operate',width:80,align:'center',formatter:formatOperate">操作</th>
				</tr>
			</thead>
		</table>
...
<div id="divAddEdit" class="easyui-dialog" title="导入账本"
	...
	<form id=formAddEdit method="post" enctype="multipart/form-data" action="<%=path%>/movies/save.html">
		<table>
			...
			<tr>
				<td><font color="red">* </font>视频:</td>
				<td><input class="easyui-filebox"  data-options="prompt:'上传视频文件'" name="video" id="video" style="width:100%"></td>
				
			</tr>
			<tr>
				<td><font color="red">* </font>视频图片:</td>
				<td><input class="easyui-filebox"  data-options="prompt:'上传视频图片'" name="image" id="image" style="width:100%"></td>
				
			</tr>
			<tr>
				<td><font color="red">* </font>标签:</td>
				<td><input class="easyui-textbox" data-options="required:true" name="tag" id="tag"/></td>
...
function formatOperate(value, row) {
	...
	if(rows.status==1)
	{
		...
		oper = "<a href='javascript:void(0)' class='easyui-linkbutton'>转换中</a>";
	}
		return oper;
}
...
function getEditFormData() {
	...
	data.no_type=form.find("#no_type").combobox('getValue');
	data.no_value=form.find("#no_type").combobox('getText');
...
function invoke(act, param, callback) {
	ddsy.core.invoke("<%=path%>/movies/" + act + ".html", param, callback);
}
