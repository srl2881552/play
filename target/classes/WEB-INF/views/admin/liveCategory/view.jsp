		...
		<table class="easyui-datagrid" id="grid" data-options="pagination:true,singleSelect:false,toolbar:'#bar',loader:loadData">
			<thead>
				<tr>
					...
					<th data-options="field:'cateName'">分类名称</th>
...
<div id="divAddEdit" class="easyui-dialog" title="添加分类"
	...
	<form id=formAddEdit method="post">
		...
		<table>
			...
			<tr>
				<td><font color="red">* </font>分类类型:</td>
				<td><select class="easyui-combobox" data-options="panelHeight:'auto',width:160,editable:true" id="type">
							<option value="1">直播分类</option>
							<option value="2">视频分类</option>
						</select></td>
...
function getQueryData() {
	var form = $("#formQuery");
	var data = {};
	data.cateName=form.find("#cate_name").val();
...
function onOk() {
	...
	if(form.find("#type").combobox('getValue') == ""){
		ddsy.msg.info("*为必填项,请填写完整");
		return false;
	}
...
function invoke(act, param, callback) {
	ddsy.core.invoke("<%=path%>/liveCategory/" + act + ".html", param, callback);
}
